from django.test import TestCase
from django.contrib.auth.models import User
from django.urls import reverse


class AuthorsLogoutTest(TestCase):
    def test_user_tries_to_logout_using_get_method(self):
        User.objects.create_user(username='my_user', password='My_p@ss1')
        self.client.login(username='my_user', password='My_p@ss1')  # This function login user automatic

        response = self.client.get(
            reverse('authors:logout'),
            follow=True  # follow redirect to authors:login
        )

        self.assertIn(
            'Invalid logout request',
            response.content.decode('utf-8')
        )

    def test_user_tries_to_logout_another_user(self):
        User.objects.create_user(username='my_user', password='My_p@ss1')
        self.client.login(username='my_user', password='My_p@ss1')  # This function login user automatic

        response = self.client.post(
            reverse('authors:logout'),
            data={
                'username': 'another_user'
            },  #
            follow=True  # follow redirect to authors:login
        )

        self.assertIn(
            'Invalid logout user',
            response.content.decode('utf-8')
        )

    def test_user_tries_to_logout_successfully(self):
        User.objects.create_user(username='my_user', password='My_p@ss1')
        self.client.login(username='my_user', password='My_p@ss1')  # This function login user automatic

        response = self.client.post(
            reverse('authors:logout'),
            data={
                'username': 'my_user'
            },  #
            follow=True  # follow redirect to authors:login
        )

        self.assertIn(
            'Logged out successfully',
            response.content.decode('utf-8')
        )
